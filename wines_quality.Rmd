...
```
We decide to remove density. Infact, we can observe that for each value of density there is no difference in the distribution of the quality, once we account for the numerosity of the two quality levels. On the other hand, the modal values for the other two variables differ for low and high quality wines, thus adding more useful information for the purpose of classification.
...
```
Here the correlations are displayed in ascending absolute value, and we can observe that the variable showing the highest correlation with wines quality is alcohol, but it is not too strong.
The sign is positive, meaning that as alcohol increases the quality of wine also improves.

### Boxplots

We now look at the distribution of all the explanatory variables, in order to both visualize those values which are far from the bulk of the data, and to get an understanding of the variables displaying the most divergent median values for low and high quality wines.
```{r, message=FALSE, warning = FALSE}
summary(wwines)
# melt the dataset
...
We now prepare the data, splitting it in training and test set, in order to apply the logistic regression.
...
To find the main factors of wine quality and remove the unrelated ones, we use forward stepwsie method to do variable selection. The best subset of the model can be found by starting from a null model with no predictors, then picking the best p (p is the number of the predictors in the full model) models which have exactly 1, 2, ..., p predictors,  and finally selecting the model with lowest Bayesian information criterion (BIC) value. In our case, the inclusin of variable follows the order of alcohol, volatile.acidity, residual.sugar, sulphates, fixed.acidity, free.sulfur.dioxide, total.sulfur.dioxide, pH, chlorides and citric.acid. When the model contains 6 variables, it has lowest BIC value. So, variables of the optimal model are the first six variables.
```{r}
library(leaps)
...
We plot the result in order to visualize which variables have been selected with BIC in each model and we highlight the optimal model with a yellow line. 
...
Since the choice of the threshold strictly depends on the aim of the analysis, which in our case is to build a predictor which would have the best behaviour in classifying new different data, we looked for a threshold which could guarantee an equilibrium between the two types of errors.
...
We now check with the Confusion Matrix the accuracy, sensitivity and sensibility of our classification model. 
...
```
We can state that:
- the model predicts wines quality with 71.00% accuracy 
- 71.55% wines are correctly identified as high quality wines
- 69.92% wines are correctly identified as low quality wines 
...
Checking the VIF after the removal of the variable "density" we can confirm that no other variable is influenced by the others.
...
As a general rule of thumb the observations with a Cook’s Distance higher than 3 times the mean, μ, are a possible outlier.
...
Again, it is useful to know if this procedure has helped improving our model, therefore here, we measure the performance on the outlier free model. 
```{r}
...
In the following table we show the comparison between the model obtained just above and the base model from which we started.
```{r}
...
```
It seems like removing the outlier does slightly improve the performance, therefore we keep using the outlier-free model from now on.
...
```
We can notice that the coefficients don't change that much when removing outliers. The only one that shifts a bit more is the coefficient relative to the intercept, which has a -0.11 change. We could assume that the outlier we removed affected the intercept coefficient estimate the most.

# OPTIMAL THRESHOLD

After removing the outliers we are worried about the goodness of the threshold we have found previously, thus we test it again.
```{r}
...
```
In our analysis, the labels are imbalanced, so we try to use bagging to improve the situation. We resample 10 subsets from training set, train the model we have found in previous step on them and aggregate the 10 models . From the result, we can see that the accuracy improves.
