...
From the result, we can see that the accuracy, sensitivity and specificity all improve.
```{r}
...
pro <- function(i){ # predict the probability
  l_p = co$mean[1]+co$mean[2]*dtrain_of[i,1]+co$mean[3]*dtrain_of[i,2]+co$mean[4]*dtrain_of[i,3]+co$mean[5]*dtrain_of[i,4]+co$mean[6]*dtrain_of[i,5]+ co$mean[7]*dtrain_of[i,6] # linear predictor
...
for (i in seq(0.01, 0.98, 0.01)){
  ...
  cm <- confusionMatrix(as.factor(lr_b_pre),dtrain_of$quality, positive = "high")
...
pro_t <- function(i){ # predict the probability
...
pre_t <- function(i){
  lr_b_pred <- ifelse(pro_t(i) < th_b,"high","low")
  return(lr_b_pred)
}
...
cm_b<-confusionMatrix(as.factor(pred_t),dtest.res$quality, positive = "high")
cm_b
...
pred_lr <- predict(lr_fit_outlierfree,  dtest.res[,1:7], type="response")
# ROC
pred <- numeric()
prob <- numeric()
for (i in rownames(dtest.res)){
...
score_data %>% 
  ...
  geom_point() + 
  geom_rug() +
  ggtitle("Both link and response scores of Test set")
```
