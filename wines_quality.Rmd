...
The Variance Inflation Factor measures how much the behaviour (variance) of an independent variable is influenced, or inflated, by its interaction/correlation with the other independent variables. Multicollinearity would not reduce the predictive power of the model, but it inflates the variance and thus the standard errors of the estimates, thus leading to wider confidence intervals, and less statistically reliable estimates.
...
We resample 500 subsets from training set, train the model we have found in previous step on them and aggregate the 10 models to get bagging model.\ 
...
# ROC
ROCit_obj <- rocit(dtest_of$probability,class=dtest_of$label)
plot(ROCit_obj)
